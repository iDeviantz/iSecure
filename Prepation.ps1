<#
 ...
 Note: This script requires elevated permissions to run successfully. i.e -Run As Administrator-
    ...
    If you are deploying the solution on a -new subscription- you will need to run the script with the 'configureGlobalAdmin' switch - otherwise the script will throw a validation error for providing additional parameters. 
	    Example - .\Preparation.ps1 -azureADDomainName .onmicrosoft.com -tenantId xxxxxx-9c8f-4e1e-941b-xxxxxx -subscriptionId xxxxx-f760-4a7e-bd98-xxxxxxxx -configureGlobalAdmin
    ...
    This script will auto generate a Global Admin as 'admin+(2-char, random number between 10-99)@azureADDomainName' and a strong, 15-char password for the account if the -configureGlobalAdmin switch is used. 
        For example - Username - WWAZUADEadmin@domain.com ; Password (will be printed to console if -configureGlobalAdmin switch is used)
#>
...
Begin{
    ...
    # Azure AD username
    $globalADAdminUserName = "WWAZUADEadmin"+(Get-Random -Maximum 99) +"@"+$azureADDomainName # e.g. WWAZUADEadmin@domain.com
...
Process
{
    ...
    {   
        ...
        try {
            ...
            $adAdmin = New-AzureADUser -DisplayName "Compliance Blueprint" -PasswordProfile $newUserPasswordProfile -AccountEnabled $true `
            -MailNickName "GRC" -UserPrincipalName $globalADAdminUserName
            Start-Sleep -Seconds 10
...
End
{
    if($configureGlobalAdmin){
        ...
        Write-Host -ForegroundColor Green " Preparation Script complete"
